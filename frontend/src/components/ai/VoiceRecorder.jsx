...
const VoiceRecorder = ({ onRecordingComplete, maxDuration = 120, autoStart = false }) => {
  ...
  const finalTranscriptRef = useRef('')   // accumulates finalized speech
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      // Auto-restart on end so pauses don't stop recognition
      recognition.onend = () => {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        setAudioURL(url)
        stopTimer()
        onRecordingComplete?.(blob, url, finalTranscriptRef.current.trim())
      }
    ...
    <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-6">
      ...
      <div className="flex justify-center gap-3 mb-4">
        ...
        {isRecording && (
          <button
            ...
            className="px-6 py-3 bg-gray-700 hover:bg-gray-800 text-white rounded-lg flex items-center gap-2 font-medium transition-all"
          >
            ⏹ Complete Answer
          </button>
        )}
      ...
      {isRecording && (
        <div className="flex justify-center items-center gap-2 mb-3">
          ...
          <span className="text-sm text-red-500 font-medium ml-2">Recording... click "Complete Answer" when done</span>
        </div>
      )}

      {/* Always show transcript box while recording */}
      {isRecording && (
        <div className="mt-3 bg-white dark:bg-gray-800 rounded-lg p-4 border-2 border-indigo-200 dark:border-indigo-700 min-h-[80px]">
          <p className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 mb-1">Live Transcript:</p>
          {displayTranscript ? (
            <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{displayTranscript}</p>
          ) : (
            <p className="text-sm text-gray-400 italic">Start speaking — your words will appear here...</p>
          )}
        </div>
      )}
      ...
      {!isRecording && displayTranscript && (
        <div className="mt-3 bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
          <p className="text-xs font-semibold text-gray-500 mb-1">Your Answer (Transcript):</p>
          <p className="text-sm text-gray-700 dark:text-gray-300 italic">{displayTranscript}</p>
        </div>
      )}
    </div>
  )
